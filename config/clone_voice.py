...
def trim_trailing_silence(samples, sample_rate, silence_thresh=0.010, min_silence_dur=0.50, pad=0.20):
    import numpy as np
    win_size = int(sample_rate * 0.05) # 50ms
    n_windows = len(samples) // win_size
    silence_count = 0
    speech_started = False
    speech_end_idx = len(samples)
    for i in range(n_windows):
        ...
        if rms > silence_thresh:
            speech_started = True
            silence_count = 0
            speech_end_idx = min(len(samples), (i + 1) * win_size + int(sample_rate * pad))
        else:
            if speech_started:
                silence_count += 1
                if silence_count * 0.05 >= min_silence_dur:
                    return samples[:speech_end_idx]
    return samples[:speech_end_idx]
